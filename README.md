...
## Почему отличается количество документов в интерфейсе и по запросу `_count`

### Пример

- **В интерфейсе:**  
  Для индекса `index` отображается **2 346 000** документов.
- **Через curl:**  
  ```
  curl -X GET "http://1.1.1.1:9200/index/_count" -u admin:pwd | jq
  ```
  Ответ:
  ```json
  {
...
- В интерфейсе используется API:  
  ```
  GET /_cat/indices?format=json
  ```
  и поле `docs.count`.

- Вручную вы используете:  
  ```
...
- **`docs.count`** из `_cat/indices` — это количество документов, включая те, которые были удалены, но ещё не физически убраны (soft deletes).
- **`_count`** — это точное количество реально доступных для поиска документов.

---

### Как правильно показывать количество документов?

- **Для мониторинга** (быстро, но не всегда точно):  
  Используйте `docs.count` из `_cat/indices`.
- **Для точных отчётов** (медленно, но точно):  
  Используйте отдельный запрос `/{index}/_count` для каждого индекса.

---

### Рекомендация

- Для дашборда и мониторинга обычно достаточно `docs.count`.
- Для отчётов и точных данных используйте `_count`.
...
|------------------|--------------|-----------------------------------------------|
| `_cat/indices`   | 2 346 000     | Быстро, но может включать soft deletes        |
| `/{index}/_count`| 1 266 000     | Медленно, но всегда точно                     |

---

**Если нужно реализовать точный подсчёт через `_count` для каждого индекса — это возможно, но будет медленнее.**
